...
function pickDivisibleCoinsForAmount(conn, objAsset, arrAddresses, last_ball_mci, amount, bMultiAuthored, spend_unconfirmed, onDone){
	...
	function pickOneCoinJustBiggerAndContinue(){
		...
		conn.query(
			...
			[arrSpendableAddresses, (!conf.free) ? amount : amount+is_base*TRANSFER_INPUT_SIZE],
			function(rows){
				if (rows.length === 1){
					var input = rows[0];
			...
			function(rows){
				async.eachSeries(
					...
					function(row, cb){
						...
						required_amount += (!conf.free) ? 0 : is_base*TRANSFER_INPUT_SIZE;
						addInput(input);
	...
	function addMcInputs(type, input_size, max_mci, onStillNotEnough){
		async.eachSeries(
			arrAddresses,
			function(address, cb){
				var target_amount = (!conf.free) ? 0 : required_amount + input_size + (bMultiAuthored ? ADDRESS_SIZE : 0) - total_amount;
