					...
					async.eachSeries(
						...
						function(objJoint, cb2){
							...
							if (assocExistingUnits[unit]){
								...
								//TODO 觸發時間  更新數據庫信息數據
								// require("./edu/eduService").updateTransaction(unit,sequence=="good"?1:2);
								// console.log("====================" ,JSON.stringify(objUnit))
								// console.log("====================" ,JSON.stringify(objUnit.messages[1]))
								let fromAddr ;
								let toAddr	;
								try{
									fromAddr = objJoint.unit.messages[1].payload.outputs[0].address;
									toAddr	 =objJoint.unit.messages[1].payload.outputs[1].address;
								}catch (e) {
									fromAddr = objJoint.unit.messages[0].payload.outputs[0].address;
									toAddr	 =objJoint.unit.messages[0].payload.outputs[1].address;
								}

								var data = {"hash":unit,"result":sequence=="good"?"1":"2","fromAddr":fromAddr,"toAddr":toAddr};
								let headers = {'API-Key': 'foobar', "Content-type": 'application/json'};

								let url = require("./conf").bizUrl;

								console.info("sendMessage to " + url + ":data :" + JSON.stringify(data))

								require("./webhelper").httpPost(url+"/bizchain/v1.0/trans/sendresult",headers,data)
									.then(function (res) {
										console.log("发送交易结果成功",res);
									})
									.catch(function (err) {
										console.log("发送交易结果失败");
									});
